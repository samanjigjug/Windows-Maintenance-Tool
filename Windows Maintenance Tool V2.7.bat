...
echo ======================================================
echo            WINDOWS MAINTENANCE TOOL V2.7 - By Lil_Batti
echo ======================================================
echo Select a command by entering a number:
echo.
...
echo [5] Clear DNS Cache (ipconfig /flushdns)
echo [6] Show network information (ipconfig /all)
echo [7] Disk Cleanup (cleanmgr)
echo [8] Network Repair - Automatic Troubleshooter
echo [9] Restart Network Adapters
...
for /f "tokens=*" %%A in ('reg query HKLM\Software\Microsoft\Windows\CurrentVersion\Uninstall 2^>nul') do (
    ...
    :: Tjek om nøglen er sikker at slette
